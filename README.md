...
- Build: npm install
- Start: npm start
- Set NODE_ENV=production and other secrets in Render UI.

Если хотите — подготовлю автоматический workflow (.github/workflows) или Dockerfile для выбранной платформы.

---

## Deploy to Render (one-service for both frontend + backend)

1. Push repository to GitHub (make sure `.env` and `users.json` are in `.gitignore`; keep `.env.example` and `users.example.json` in repo).
2. Option A — Manual (UI):
   - Sign in to Render and click "New" → "Web Service".
   - Connect GitHub and select your repo + branch (main).
   - Build command: `npm install`
   - Start command: `npm start`
   - Environment: `Node`, set env var `NODE_ENV=production` and any secrets (e.g. `JWT_SECRET`).
   - Create service — Render will build and provide a public URL where both the frontend (static files) and API are available.

3. Option B — Declarative (render.yaml)
   - Add `render.yaml` to repo (example provided) and push — Render can auto-detect and create the service.
   - After deploy, open the service URL and test:
     - Frontend: open root URL in browser.
     - API: `POST https://your-service-url/api/login` etc.

Notes:
- Server must listen on `process.env.PORT` (server.js already uses it).
- For local testing: `npm install` → `npm start`.
- If you prefer separate hosting: host static on Netlify/GitHub Pages and backend on Render; remember to set API URL in frontend and configure CORS if needed.
     - API: `POST https://your-service-url/api/login` etc.
